...
\begin{frame}[fragile]
    ...
    所以在C++中的加减乘除（单双目）、与或非、大小等、赋值在Python中也适用，但Python还支持一些额外的运算符，如幂运算符\mintinline{python3}{**}、取模运算符\mintinline{python3}{\%}、整除运算符\mintinline{python3}{//}等。
...
class Person:
    ...
    \end{minted}
    在这个例子中，我们定义了一个名为\mintinline{python3}{Person}的类，它有一个构造函数\mintinline{python3}{__init__}，用于初始化实例变量\mintinline{python3}{name}和\mintinline{python3}{age}，还有一个方法\mintinline{python3}{greet}，用于打印一个问候语。
...
\begin{frame}[fragile]
    ...
    你可以使用\mintinline{python3}{import}语句来导入模块或包。有三种常见的导入方式：
    \begin{minted}{python3}
import math  # 导入math模块，之后调用应math.sqrt()等，相当于#include <cmath>;
from math import sqrt  # 从math模块导入sqrt函数，之后可以直接调用sqrt()函数，相当于using std::sqrt;
from math import *  # 从math模块导入所有函数和变量，之后可以直接调用math模块中的所有函数和变量，如sqrt()、pi等，相当于using namespace std;
    \end{minted}
